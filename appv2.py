"""LCA fiber comparison dashboard V2 (Shiny + Plotly)."""
...
BARCHART_TEST_NORM_TAB = "All Potential Environmental Impact"
BARCHART_TEST_NORM_TAB_ID = "barchart_test_norm"
BARCHART_VER_TEST_TAB = "All Potential Environmental Impact Horizontal View"
BARCHART_VER_TEST_TAB_ID = "barchart_ver_test"
BARCHART_TEST_NORM_METHOD = NORM_METHOD_PER_IMPACT


def _normalization_supported(fiber_count: int, *, method: str) -> bool:
...
def _resolve_impacts_in_data(
    ...
    """Match impact names to dataset rows (case-insensitive)."""
...
def _apply_bar_y_axis_range(
    ...
    *,
    ...
    if orientation == "h":
        fig.update_xaxes(range=axis_range, autorange=False, **axis_kw)
    else:
...
def _export_plotly_image(fig: go.Figure, *, format: str) -> bytes:
    """High-resolution static export for PNG/PDF downloads."""
...
def _chart_download_buttons(plot_output_id: str) -> ui.Tag:
    return ui.div(
        #  ui.download_button(
        #     f"{plot_output_id}_download_png",
        #     "PNG",
...
def _register_chart_download_handlers(
    ...
    *,
    ...
    png_id = f"{plot_id}_download_png"
    pdf_id = f"{plot_id}_download_pdf"

    def _png_handler():
        yield _export_plotly_image(figure_fn(), format="png")

    def _pdf_handler():
        yield _export_plotly_image(figure_fn(), format="pdf")

    # Shiny registers downloads under handler.__name__ before @output(id=...) runs;
    # names must match the download button / output ids or the route 404s.
    _png_handler.__name__ = png_id
    _pdf_handler.__name__ = pdf_id

    output(id=png_id)(
        render.download(
            filename=lambda: f"{_safe_download_basename(basename_fn())}.png",
...
def _bar_axis_labels(
    ...
    """Full fiber names for data/hover; truncated names for vertical axis display."""
...
def _bar_impact_axis_labels(
    ...
    *,
    ...
    """Full and truncated impact names for grouped bar chart x-axis positions."""
    full: list[str] = []
    for impact in impacts:
        row = _row_for_impact(data, impact_col, impact)
        if row is None:
...
def build_bar_figure(
    ...
    *,
    ...
    norm_value_label = ""
    if use_normalized:
        norm_note, norm_value_label = _norm_chart_labels(norm_method, baseline)
    ...
    impact_full, impact_display = _bar_impact_axis_labels(impacts, data, impact_col)
    if horizontal:
        bottom_margin = 50
        left_margin = _bar_y_label_width_px(impact_full)
    else:
        bottom_margin = _bar_x_label_height_px(impact_full)
    ...
    for fiber_idx, fiber in enumerate(fibers):
        ...
        for impact in impacts:
            ...
            if row is None:
                y_vals.append(None)
                continue
            impact_values = _values_for_impact(impact, row)
            y_vals.append(
    ...
    value_title = norm_value_label if use_normalized else BAR_Y_LABEL_RAW
    if horizontal:
        row_px = max(36, 22 * len(fibers))
        height = max(layout.single_height, len(impact_full) * row_px + 120)
        margin = {**layout.margin_single, "l": left_margin, "b": bottom_margin, "r": 30, "t": 72}
        fig.update_layout(
            height=height,
            autosize=True,
            margin=margin,
...
def _barchart_ver_test_guide_panel_ui() -> ui.Tag:
    body = ui.div(
        ...
        ui.tags.ul(
            ...
            ui.tags.li(
                "Values are max-abs normalized within each impact row: "
                "each fiber value is divided by the largest absolute value "
                "among compared fibers (range −1 to 1).",
            ),
            ui.tags.li(
    ...
    return _hover_panel_ui(
        ...
        title="Horizontal bar chart (test normalization)",
        body=body,
    )


def _barchart_ver_test_nav_panel() -> ui.Tag:
    return ui.nav_panel(
        BARCHART_VER_TEST_TAB,
        ui.card(
            ui.card_header(BARCHART_VER_TEST_TAB),
            ui.div(
                ui.tags.p(
                    "Per-impact normalization (−1 to 1), shown as horizontal bars. "
                    "Each impact row is scaled independently.",
                    class_="text-muted small mb-0",
                ),
                ui.div(
                    _barchart_ver_test_guide_panel_ui(),
                    class_="d-flex flex-wrap align-items-center gap-3 px-2 pt-2 pb-1",
                ),
                class_="px-2 pt-2",
            ),
            _chart_plot_output("barchart_ver_test_plot"),
            full_screen=True,
        ),
        value=BARCHART_VER_TEST_TAB_ID,
    )


def _barchart_test_norm_nav_panel() -> ui.Tag:
    return ui.nav_panel(
        BARCHART_TEST_NORM_TAB,
        ui.card(
            ui.card_header(BARCHART_TEST_NORM_TAB),
            ui.div(
                ui.tags.p(
                    "Per-impact normalization (−1 to 1). "
                    "Each impact row is scaled independently.",
                    class_="text-muted small mb-0",
                ),
                ui.div(
                    _barchart_test_norm_guide_panel_ui(),
                    class_="d-flex flex-wrap align-items-center gap-3 px-2 pt-2 pb-1",
                ),
                class_="px-2 pt-2",
            ),
            _chart_plot_output("barchart_test_norm_plot"),
            full_screen=True,
        ),
        value=BARCHART_TEST_NORM_TAB_ID,
    )
...
def _select_card_panel(
    ...
    *,
    ...
    card_attrs: dict[str, str] = {"class_": "lca-checkbox-card"}
    if card_id:
        card_attrs["id"] = card_id
    return ui.card(header, body, **card_attrs)
...
.lca-baseline-inactive {
    opacity: 0.72;
}
.lca-baseline-inactive .lca-checkbox-panel-title {
    color: #6c757d;
}
.lca-baseline-inactive .selectize-control {
    pointer-events: none;
}
.lca-baseline-inactive .selectize-control.single .selectize-input {
    ...
    for _, row in table.iterrows():
        ...
        for fiber in fiber_cols:
            ...
            if _json_safe_number(value) is None:
                ...
                elif value_format in ("ratio", "asinh"):
...
def _normalization_hover_panel_ui(
    *,
    ...
    body = ui.div(
        ui.tags.p(
            "Choose a normalization method. Baseline uses the fiber selected under "
    ...
    body = ui.div(
        ...
        ui.tags.ul(
            ...
            ui.tags.li(
                ...
                " below a bar (single impact) or gray bars (grouped) mean "
                "that fiber has no value (N/A) for that impact.",
            ),
            ui.tags.li(
...
def _barchart_test_norm_guide_panel_ui() -> ui.Tag:
    body = ui.div(
        ui.tags.p("How to read this chart:", class_="lca-formula-lead mb-2"),
        ui.tags.ul(
            ui.tags.li(
    ...
    return _hover_panel_ui(
        trigger_label="Bar Chart guide",
        icon_svg=_GUIDE_ICON_SVG,
        title="Bar chart (test normalization)",
        body=body,
    )
...
"""),
    ...
    ui.navset_tab(
        ui.nav_panel(
            "Comparision Table",
            ui.card(
                ui.card_header("Comparison Table - FU=100kg"),
                ui.tags.p(
                    "Select fibers and impacts in the sidebar, then click Apply.",
                    class_="text-muted small px-3 pt-2 mb-0",
                ),
                _comparison_table_section("Raw data", "data_tab_raw"),
                _comparison_table_section(
                    "Normalized (per impact, −1 to 1)",
                    "data_tab_normalized",
                ),
                full_screen=True,
            ),
        ),
        _barchart_test_norm_nav_panel(),
        _barchart_ver_test_nav_panel(),
        ui.nav_panel(
            "About",
        ),
        id="main_tabs",
...
def server(input, output, session):
    ...
    def _on_apply_settings() -> None:
        _live_fiber_cache.set(list(input.fiber() or []))
        _live_impact_cache.set(list(input.impact() or []))
        _commit_applied_state(_capture_applied_state())
    ...
    def _sync_checkbox_groups(
        ...
        _commit_applied_state(
            AppliedState(fibers=applied_fibers, impacts=applied_impacts)
        )
    ...
    _register_table_csv_download_handler(
        ...
        basename_fn=lambda: "lca-comparison-raw",
    )
    _register_table_csv_download_handler(
        output,
        render,
        ...
        basename_fn=lambda: "lca-comparison-normalized",
    )

    @reactive.calc
    def barchart_test_norm_figure() -> go.Figure:
        state = _applied_state.get()
        data = current_data()
        return build_bar_figure(
            list(state.impacts),
            ...
            data.df,
            data.impact_col,
            norm_method=BARCHART_TEST_NORM_METHOD,
            baseline="",
        )

    @output(id="barchart_test_norm_plot", suspend_when_hidden=True)
    @render_plotly
    def barchart_test_norm_plot():
        return _plotly_widget(barchart_test_norm_figure())

    _register_chart_download_handlers(
        output,
        render,
        plot_id="barchart_test_norm_plot",
        figure_fn=barchart_test_norm_figure,
        basename_fn=lambda: "lca-barchart-test-norm",
    )

    @reactive.calc
    def barchart_ver_test_figure() -> go.Figure:
        state = _applied_state.get()
        data = current_data()
        return build_bar_figure(
            list(state.impacts),
    ...
    @output(id="barchart_ver_test_plot", suspend_when_hidden=True)
    @render_plotly
    def barchart_ver_test_plot():
        return _plotly_widget(barchart_ver_test_figure())

    _register_chart_download_handlers(
        output,
        render,
        plot_id="barchart_ver_test_plot",
        figure_fn=barchart_ver_test_figure,
        basename_fn=lambda: "lca-barchart-ver-test",
    )
